...
public class AuctionMarkProfile {
  ...
  /**
   * We maintain a cached version of the profile that we will copy from This prevents the need to
   * have every single client thread load up a separate copy
   */
  ...
  /**
   * The total number of clients in this benchmark invocation. Each client will be responsible for
   * adding new auctions for unique set of sellers This may change per benchmark invocation.
   */
  private final transient int num_clients;
  ...
  /**
   * The stop time for when the loader was finished We can reset anything that has a timestamp after
   * this one
   */
  private Timestamp loaderStopTime;

  /**
   * A histogram for the number of users that have the number of items listed ItemCount -> # of
   * Users
   */
  ...
  /**
   * Three status types for an item: (1) Available - The auction of this item is still open (2)
   * Ending Soon (2) Wait for Purchase - The auction of this item is still open. There is a bid
   * winner and the bid winner has not purchased the item. (3) Complete (The auction is closed and
   * (There is no bid winner or the bid winner has already purchased the item)
   */
  private final transient ItemInfoList items_available = new ItemInfoList();

  private final transient ItemInfoList items_endingSoon = new ItemInfoList();
  private final transient ItemInfoList items_waitingForPurchase = new ItemInfoList();
  private final transient ItemInfoList items_completed = new ItemInfoList();

  protected final transient ItemInfoList allItemSets[] =
      new ItemInfoList[] {
  ...
  public final transient DiscreteRNG randomTimeDiff;

  /** Random duration in days */
  public final transient Gaussian randomDuration;

  protected final transient Zipf randomNumImages;
  protected final transient Zipf randomNumAttributes;
  protected final transient Zipf randomPurchaseDuration;
  protected final transient Zipf randomNumComments;
  protected final transient Zipf randomInitialPrice;
  ...
  private final transient Timestamp lastCloseAuctionsTime = new Timestamp(0);

  /** When this client started executing */
  private final transient Timestamp clientStartTime = new Timestamp(0);

  /** Current Timestamp */
  private final transient Timestamp currentTime = new Timestamp(0);

  /** TODO */
  protected final transient Histogram<UserId> seller_item_cnt = new Histogram<>();

  /** TODO */
  protected final transient List<ItemCommentResponse> pending_commentResponses = new ArrayList<>();

  // -----------------------------------------------------------------
  // TEMPORARY VARIABLES
  // -----------------------------------------------------------------

  private final transient Set<ItemInfo> tmp_seenItems = new HashSet<>();
  private final transient Histogram<UserId> tmp_userIdHistogram = new Histogram<>(true);
  private final transient Timestamp tmp_now = new Timestamp(System.currentTimeMillis());
  ...
  protected final void saveProfile(Connection conn) throws SQLException {
    ...
    // CONFIG_PROFILE
    Table catalog_tbl =
        this.benchmark.getCatalog().getTable(AuctionMarkConstants.TABLENAME_CONFIG_PROFILE);

    String sql =
        SQLUtil.getInsertSQL(
            catalog_tbl, this.benchmark.getWorkloadConfiguration().getDatabaseType());
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      ...
      stmt.setObject(
          param_idx++, this.users_per_itemCount.toJSONString()); // CFP_USER_ITEM_HISTOGRAM
  ...
  private void initializeUserIdGenerator(int clientId) {
    ...
    this.userIdGenerator =
        new UserIdGenerator(
            this.users_per_itemCount, this.num_clients, (clientId < 0 ? null : clientId));
  }
  ...
  private static void loadItemCategoryCounts(AuctionMarkProfile profile, List<Object[]> vt) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          String.format(
              "Loaded %d CATEGORY records from %s",
  ...
  private ItemStatus addItemToProperQueue(
      ItemInfo itemInfo, Timestamp baseTime, Iterator<ItemInfo> currentQueueIterator) {
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          String.format(
              "%s - #%s [%s]", new_status, itemInfo.getItemId().encode(), itemInfo.getEndDate()));
    }
  ...
  private ItemInfo getRandomItem(
      LinkedList<ItemInfo> itemSet, boolean needCurrentPrice, boolean needFutureEndDate) {
    Timestamp currentTime = this.updateAndGetCurrentTime();
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          String.format(
              "Getting random ItemInfo [numItems=%d, currentTime=%s, needCurrentPrice=%s]",
    ...
    if (itemInfo == null) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Failed to find ItemInfo [hasCurrentPrice={}, needFutureEndDate={}]",
            needCurrentPrice,
            needFutureEndDate);
      }
      return (null);
