...
public final class AuctionMarkWorker extends Worker<AuctionMarkBenchmark> {
  ...
  private class CloseAuctionsChecker extends Thread {
    ...
    public void run() {
      ...
      long sleepTime =
          AuctionMarkConstants.CLOSE_AUCTIONS_INTERVAL / AuctionMarkConstants.TIME_SCALE_FACTOR;
      while (true) {
  ...
  public interface AuctionMarkParamGenerator {
    /**
     * Returns true if the client will be able to successfully generate a new transaction call The
     * client passes in the current BenchmarkProfile handle and an optional VoltTable. This allows
     * you to invoke one txn using the output of a previously run txn. Note that this is not thread
     * safe, so you'll need to combine the call to this with generate() in a single synchronization
     * block.
     *
     * @param client
     * @return
  ...
  public enum Transaction {
    ...
    protected static final Map<Class<? extends Procedure>, Transaction> class_lookup =
        new HashMap<>();
    ...
    /**
     * This will return true if we can call a new transaction for this procedure A txn can be called
     * if we can generate all of the parameters we need
     *
     * @return
     */
  ...
  protected TransactionStatus executeWork(Connection conn, TransactionType txnType)
    ...
    // since that accounts for the time gap between when the loader started and when the client
    // start.
  ...
  /**
   * For the given VoltTable that contains ITEM records, process the current row of that table and
   * update the benchmark profile based on item information stored in that row.
   *
   * @param row
   * @return
   * @see CloseAuctions
  ...
  public ItemId processItemRecord(Object[] row) {
    ...
    if (i_end_date == null) {
      LOG.warn(
          "end date is null: {} / {} expected timestamp or date",
          row[col - 1],
          row[col - 1].getClass());
    }
  ...
  public Timestamp[] getTimestampParameterArray() {
    return new Timestamp[] {profile.getLoaderStartTime(), profile.getClientStartTime()};
  }
  ...
  protected boolean executeGetItem(Connection conn, GetItem proc) throws SQLException {
    ...
    Object[][] results =
        proc.run(
            conn, benchmarkTimes, itemInfo.getItemId().encode(), itemInfo.getSellerId().encode());
  ...
  protected boolean executeGetUserInfo(Connection conn, GetUserInfo proc) throws SQLException {
    ...
    boolean get_watched_items =
        (rand <= AuctionMarkConstants.PROB_GETUSERINFO_INCLUDE_WATCHED_ITEMS);
  ...
  protected boolean executeNewBid(Connection conn, NewBid proc) throws SQLException {
    ...
    if ((has_waiting || has_completed)
        && (profile.rng.number(1, 100) <= AuctionMarkConstants.PROB_NEWBID_CLOSED_ITEM
            || !has_available)) {
      if (has_waiting) {
    ...
    // Otherwise we want to generate information for a real bid
      ...
      bid =
          profile.rng.fixedPoint(
              2,
              currentPrice,
              currentPrice * (1 + (AuctionMarkConstants.ITEM_BID_PERCENT_STEP / 2)));
      maxBid =
          profile.rng.fixedPoint(
              2, bid, (bid * (1 + (AuctionMarkConstants.ITEM_BID_PERCENT_STEP / 2))));
    }
  ...
  protected boolean executeNewComment(Connection conn, NewComment proc) throws SQLException {
    ...
    profile.pending_commentResponses.add(
        new ItemCommentResponse(
            SQLUtil.getLong(results[0]),
  ...
  protected boolean executeNewCommentResponse(Connection conn, NewCommentResponse proc)
      throws SQLException {
    Timestamp[] benchmarkTimes = this.getTimestampParameterArray();
    ...
    proc.run(conn, benchmarkTimes, itemId.encode(), sellerId.encode(), commentId, response);
  ...
  protected boolean executeNewItem(Connection conn, NewItem proc) throws SQLException {
    ...
    try {
      ...
      LOG.warn(
          "a duplicate item existed; i believe this error should be handled differently.: "
              + ex.getMessage());
      throw ex;
    }
  ...
  protected boolean executeNewPurchase(Connection conn, NewPurchase proc) throws SQLException {
    ...
    String ip_id = AuctionMarkUtil.getUniqueElementId(encodedItemId, ip_id_cnt);
    ...
    Object[] results =
        proc.run(conn, benchmarkTimes, encodedItemId, sellerId.encode(), ip_id, buyer_credit);
